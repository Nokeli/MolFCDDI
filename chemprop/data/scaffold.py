...
def scaffold_to_smiles(mols: Union[List[str], List[Chem.Mol]],
    ...
    return scaffolds
    # scaffolds = defaultdict(set)
    # for i, mol1,mol2 in enumerate(mols):
    #     scaffold1 = generate_scaffold(mol1)
    #     scaffold2 = generate_scaffold(mol2)
    #     if use_indices:
    #         scaffolds[(scaffold1,scaffold2)].add(i)
    #     else:
    #         scaffolds[(scaffold1,scaffold2)].add((mol1,mol2))
    # return scaffolds
...
def scaffold_split(data: MoleculeDataset,
    ...
    train, val, test = [], [], []
    train_scaffold_count, val_scaffold_count, test_scaffold_count = 0, 0, 0
    ddi_set = data.mols_pair()
    unique_drugs = set()
    for d1,d2 in ddi_set:
        unique_drugs.add(d1)
        unique_drugs.add(d2)
    # Map from scaffold to index in the data
    scaffold_to_indices = scaffold_to_smiles(data.mols_pair(), use_indices=True)
