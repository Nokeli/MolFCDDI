    ...
    if dataset_type == 'classification' or dataset_type == 'multiclass':
        ...
        try:
            if dataset_type == 'multiclass':
                ...
                auc = roc_auc_score(targets_binarized, pred_probs, average='macro', multi_class='ovr')
            else:
                # For binary classification, use probability of positive class (column 1)
                positive_probs = pred_probs[:, 1] if pred_probs.ndim > 1 else pred_probs
                auc = roc_auc_score(targets_array, positive_probs)
        ...
        try:
            if dataset_type == 'multiclass':
                ...
                aupr = average_precision_score(targets_binarized, pred_probs, average='macro')
            else:
                # For binary classification, use probability of positive class (column 1)
                positive_probs = pred_probs[:, 1] if pred_probs.ndim > 1 else pred_probs
                aupr = average_precision_score(targets_array, positive_probs)
        except ValueError as e:
        ...
        try:
            if dataset_type == 'multiclass':
                ...
                f1 = f1_score(targets_array, pred_labels, average='binary')
        except ValueError as e:
