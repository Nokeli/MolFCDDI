    ...
    def forward(self, step, mol_graph, features_batch=None) -> torch.FloatTensor:
        ...
        if self.args.step == 'func_prompt':  # 使用高层次BRICS片段进行反应
            ...
            if self.args.add_step == 'func_frag_attention':
                ...
                pharm_hiddens, self.self_att = self.W_i_atom.prompt_generator(input_atom, a_scope, f_group,
                                                                              mapping_scope, func2atom, func2atom_scope)
                ...
                pharm_hiddens, self.self_att = self.W_i_atom.prompt_generator(None, input_atom, a_scope, f_group,
                                                                              group_scope, mapping, mapping_scope)
        ...
        if self.args.step == 'func_prompt' and self.args.add_step == 'concat_mol_frag_attention':
            f_group = self.funcional_group_embedding(mapping)  # get random functional group embedding
            f_out, self.self_att = self.W_i_atom.prompt_generator(atom_hiddens, a_scope, f_group, mapping_scope,
                                                                          func2atom, func2atom_scope)
            pharm_hiddens = f_out[:,0,:]
            mol_vecs_fp = self.act_func(self.W_molecular(torch.cat([mol_vecs, pharm_hiddens], dim=-1)))  # 300
            mol_vecs_fp = self.dropout_layer(mol_vecs_fp)
        return f_out, mol_vecs, mol_vecs_fp 
...
class CMPN(nn.Module):
    ...
    def forward(self, step, pretrain: bool, batch,
        ...
        #     batch = mol2graph_optimized(batch, self.args, pretrain)
        fp,output,output_fp = self.encoder.forward(step, batch, features_batch)
        return fp,output, output_fp